...
/**
 * Avenger custom client
 * @extends {Client}
 */
class Avenger extends Client {
	constructor() {
		super({
	...
	SetActivity(type, array = []) {
		...
		try {
			...
			setInterval(
				() =>
					this.user.setActivity(`${this.Activity[j++ % this.Activity.length]}`, {
						type: type,
						url: 'https://www.twitch.tv/koushikpuppala',
					}),
				10000
	...
	async loadInteractionGroup(category) {
		try {
			const commands = (await readdir('./src/commands/' + category + '/')).filter(
				(v, i, a) => a.indexOf(v) === i
			)
			const arr = []
