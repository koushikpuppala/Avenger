...
class Meme extends Command {
	...
	async fetchMeme(bot, guild, settings) {
		try {
			const meme = await new (require('../../APIs/reddit.js'))().fetchMeme({
				removeNSFW: true,
			})
			if (!meme.imageURL) {
				...
				return new Embed(bot, guild)
					...
					.setFooter('fun/meme:FOOTER', {
						UPVOTES: meme.upvotes.toLocaleString(settings.Language),
						DOWNVOTES: meme.downvotes.toLocaleString(settings.Language),
					})
			}
		} catch (err) {
			bot.logger.error(err.message)
			bot.commands.delete('meme')
			return new Embed(bot, guild).setDescription('Meme failed to load')
		}
	}
}
