		...
		if (!num1 || !num2) {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('fun/random:USAGE')),
				})
	...
	async callback(bot, interaction, guild, args) {
			...
			interaction.reply({
				embeds: [
					channel.error(
						...
						{ EXAMPLE: settings.prefix.concat(bot.translate('fun/random:USAGE')) },
						true
					),
				],
				ephemeral: true,
		...
		return interaction.reply({
			embeds: [
				{
					color: 'RANDOM',
					description: guild.translate('fun/random:RESPONSE', { NUMBER: r }),
				},
			],
		})
	}
