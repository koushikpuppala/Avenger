...
class Reminder extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[1]) {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('fun/reminder:USAGE')),
				})
		...
		await message.channel
			.send(
				message.translate('fun/reminder:MESSAGE', {
				...
				setTimeout(async () => {
					...
					message.member.send({ embeds: [embed], files: [attachment] }).catch(() => {
						message.channel.send(
							message
								.translate('fun/reminder:RESPONSE', {
									INFO: message.args.join(' '),
								})
