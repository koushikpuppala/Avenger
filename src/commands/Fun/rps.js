...
module.exports = class RPS extends Command {
	...
	async run(bot, message, settings) {
		...
		if (
			message.args[0].includes('paper') ||
			message.args[0].includes('rock') ||
			message.args[0].includes('scissors')
		) {
			// Bot decision time
			...
			if (choice == message.args[0]) {
				...
				(choice == 'rock' && message.args[0] == 'scissors') ||
				(choice == 'paper' && message.args[0] == 'rock') ||
				(choice == 'scissors' && message.args[0] == 'paper')
			) {
				winner = 'bot'
			} else {
				winner = 'user'
			...
			// send results
			const embed = new MessageEmbed().setTitle('Rock Paper Scissors')
				.setDescription(`**${bot.translate(settings.Language, 'FUN/RPS_FIRST')}:** ${
				message.args[0]
			}
