...
class Screenshot extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0]) {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('fun/screenshot:USAGE')),
				})
		...
		if (!validUrl.isUri(message.args[0])) {
			if (message.deletable) message.delete()
			return message.channel
				.error('fun/screenshot:INVALID_URL')
				.then((m) => m.timedDelete({ timeout: 5000 }))
		}

		// Make sure website is not NSFW in a non-NSFW channel
		if (
			!bot.adultSiteList.includes(require('url').parse(message.args[0]).host) &&
			!message.channel.nsfw &&
			message.guild
		) {
			if (message.deletable) message.delete()
			return message.channel
				.error('fun/screenshot:BLACKLIST_WEBSITE')
				.then((m) => m.timedDelete({ timeout: 5000 }))
		}
		...
		if (!data) {
			return message.channel
				.error('misc:ERROR_MESSAGE', { ERROR: 'Failed to fetch screenshot' })
				.then((m) => m.timedDelete({ timeout: 5000 }))
		} else {
	...
	async callback(bot, interaction, guild, args) {
		...
		if (!validUrl.isUri(url)) {
			return interaction.reply({
				embeds: [channel.error('fun/screenshot:INVALID_URL', {}, true)],
				ephermal: true,
			})
		}
		...
		if (!bot.adultSiteList.includes(require('url').parse(url).host) && !channel.nsfw) {
			return interaction.reply({
				embeds: [channel.error('fun/screenshot:BLACKLIST_WEBSITE', {}, true)],
				ephermal: true,
			})
		}
