...
class GiveawayEdit extends Command {
	...
	constructor(bot) {
		super(bot, {
			...
			cooldown: 2000,
			examples: [
				'g-edit 818821436255895612 2m 2 nitro',
				'g-edit 818821436255895612 3h40m 5 nitro classic',
			],
		})
	}
	...
	async run(bot, message, settings) {
		...
		if (message.args.length <= 3) {
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('giveaway/g-edit:USAGE')),
				})
		...
		if (isNaN(message.args[2])) {
			return message.channel
				.error('giveaway/g-edit:INCORRECT_WINNER_COUNT')
				.then((m) => m.timedDelete({ timeout: 5000 }))
		}
		...
		bot.giveawaysManager
			...
			.then(() => {
				message.channel.send(
					bot.translate('giveaway/g-edit:EDIT_GIVEAWAY', {
						TIME: bot.giveawaysManager.options.updateCountdownEvery / 1000,
					})
				)
			})
			.catch((err) => {
				...
				message.channel.send(
					bot.translate('giveaway/g-edit:UNKNOWN_GIVEAWAY', { ID: message.args[0] })
				)
			})
	}
}
