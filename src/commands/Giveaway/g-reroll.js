		...
		if (!message.args[0]) {
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('giveaway/g-reroll:USAGE')),
				})
		...
		bot.giveawaysManager
			.reroll(messageID, {
				winnerCount: !parseInt(message.args[1])
					? bot.giveawaysManager.giveaways.find((g) => g.messageID == messageID)
							?.winnerCount
			...
			.catch((err) => {
				...
				message.channel.send(
					bot.translate('giveaway/g-reroll:UNKNOWN_GIVEAWAY', { ID: messageID })
				)
			})
	}
}
