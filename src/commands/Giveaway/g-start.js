...
class GiveawayStart extends Command {
	...
	async run(bot, message, settings) {
		...
		if (message.args.length <= 2)
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('giveaway/g-start:USAGE')),
				})
		...
		if (message.args.slice(2).join(' ').length >= 256)
			return message.channel
				.error('giveaway/g-start:PRIZE_TOO_LONG')
				.then((m) => m.timedDelete({ timeout: 5000 }))
		...
		bot.giveawaysManager
			...
			.then(() => {
				bot.logger.log(
					`${message.author.tag} started a giveaway in server: [${message.guild.id}].`
				)
			})
			.catch((err) => {
