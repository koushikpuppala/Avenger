...
class RoleInfo extends Command {
		...
		if (!roles[0]) {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('guild/role-info:USAGE')),
				})
	...
	createEmbed(bot, guild, role, user) {
		...
		const permissions = role.permissions
			.toArray()
			.map((p) => guild.translate(`permissions:${p}`))
			.join(' » ')
			...
			.addFields(
				{
					...
					value: role.members.size.toLocaleString(guild.settings.Language),
					inline: true,
				},
				{
					name: guild.translate('guild/role-info:COLOR'),
					value: role.hexColor,
					inline: true,
				},
				{
					name: guild.translate('guild/role-info:POSITION'),
					value: `${role.position}`,
					inline: true,
				},
				{
					name: guild.translate('guild/role-info:MENTION'),
					value: `<@&${role.id}>`,
					inline: true,
				},
				{
					name: guild.translate('guild/role-info:HOISTED'),
					value: `${role.hoist}`,
					inline: true,
				},
				{
					name: guild.translate('guild/role-info:MENTIONABLE'),
					value: `${role.mentionable}`,
					inline: true,
				},
				{ name: guild.translate('guild/role-info:PERMISSION'), value: permissions },
				{
					name: guild.translate('guild/role-info:CREATED'),
					value: moment(role.createdAt).format('lll'),
				}
			)
			.setTimestamp()
