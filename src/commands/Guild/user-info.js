...
class UserInfo extends Command {
	...
	createEmbed(bot, guild, member) {
		const status =
			member.presence?.activities.length >= 1
				? `${member.presence.activities[0].name} - ${
						member.presence.activities[0].type == 'CUSTOM_STATUS'
		...
		return new Embed(bot, guild)
			...
			.addFields(
				{
					name: guild.translate('guild/user-info:USERNAME'),
					value: member.user.username,
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:DISCRIM'),
					value: `${member.user.discriminator}`,
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:ROBOT'),
					value: guild.translate(`misc:${member.user.bot ? 'YES' : 'NO'}`),
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:CREATE'),
					value: moment(member.user.createdAt).format('lll'),
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:STATUS'),
					value: `\`${status}\``,
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:ROLE'),
					value: `${member.roles.highest}`,
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:JOIN'),
					value: moment(member.joinedAt).format('lll'),
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:NICK'),
					value: member.nickname != null ? member.nickname : guild.translate('misc:NONE'),
					inline: true,
				},
				{
					name: guild.translate('guild/user-info:ROLES'),
