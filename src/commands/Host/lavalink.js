...
class Lavalink extends Command {
	...
	async run(bot, message) {
		...
		if (message.args[0] == 'list') {
			...
			const embed = new Embed(bot, message.guild).setTitle('Lavalink nodes:').setDescription(
				bot.manager.nodes
					.map((node, index, array) => {
						return `${
							array.map(({ options }) => options.host).indexOf(index) + 1
						}.) **${node.options.host}** (Uptime: ${this.uptime(
							node.stats.uptime ?? 0
						)})`
					})
					.join('\n')
			...
			;(msg = await message.channel.send(message.translate('host/lavalink:FETCHING'))),
				({ memory, cpu, uptime, playingPlayers, players } = bot.manager.nodes.get(
					message.args[0]
				).stats)
		} else {
			;(msg = await message.channel.send(message.translate('host/lavalink:FETCHING'))),
		...
		const embed = new Embed(bot, message.guild)
			.setAuthor(
				message.translate('host/lavalink:AUTHOR', {
					NAME:
						bot.manager.nodes.get(message.args[0])?.options.host ??
						bot.manager.nodes.first().options.host,
				})
			)
			.addField(
				message.translate('host/lavalink:PLAYERS'),
				message.translate('host/lavalink:PLAYER_STATS', {
					PLAYING: playingPlayers,
					PLAYERS: players,
				})
			)
			.addField(
				message.translate('host/lavalink:MEMORY'),
				message.translate('host/lavalink:MEMORY_STATS', {
					ALL: allocated,
					USED: used,
					FREE: free,
					RESERVE: reservable,
			...
			.addField(
				message.translate('host/lavalink:CPU'),
				message.translate('host/lavalink:CPU_STATS', {
					CORES: cpu.cores,
					SYSLOAD: systemLoad,
					LVLLOAD: lavalinkLoad,
				})
			)
			.addField(
				message.translate('host/lavalink:UPTIME'),
				message.translate('host/lavalink:UPTIME_STATS', { NUM: botUptime })
			)
