	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('host/reload:USAGE')),
				})
		...
		if (bot.commands.has(commandName) || bot.commands.get(bot.aliases.get(commandName))) {
			...
			try {
				...
				return message.channel
					.success('host/reload:SUCCESS', { NAME: commandName })
				...
				evtFolder.forEach(async (folder) => {
					...
					folders.forEach(async (file) => {
						...
						if (name == message.args[0]) {
							...
							return message.channel
								.success('host/reload:SUCCESS_EVENT', { NAME: message.args[0] })
								.then((m) => m.timedDelete({ timeout: 8000 }))
						}
					})
				})
			...
			return message.channel
				.error('host/reload:INCORRECT_DETAILS', { NAME: commandName })
