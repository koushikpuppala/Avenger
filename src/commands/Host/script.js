	...
	async run(bot, message) {
		...
		if (!message.args[0]) {
			const embed = new MessageEmbed()
				...
				.setDescription(
					scripts.map((c, i) => `${i + 1}.) ${c.replace('.js', '')}`).join('\n')
				)
		...
		if (scripts.includes(`${message.args[0]}.js`)) {
			try {
				const resp = await require(`../../scripts/${message.args[0]}.js`).run(
					eval(message.args[1], { depth: 0 }),
					eval(message.args[2], { depth: 0 }),
					eval(message.args[3], { depth: 0 })
				)
