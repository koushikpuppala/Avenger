...
class Suggestion extends Command {
	...
	constructor(bot) {
		super(bot, {
			...
			cooldown: 3000,
			examples: [
				'suggestion Level reset - Should member levels reset when they leave the server - Economy plugin',
			],
		})
	}
			...
			if (words.length != 3)
				return message.channel
					.error('misc:INCORRECT_FORMAT', {
						EXAMPLE: settings.prefix.concat(message.translate('host/suggestion:USAGE')),
					})
