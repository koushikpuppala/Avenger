	...
	async run(bot, message, settings) {
				...
				.setDescription(
					[
						...
						`Premium: \`${user.premium}\`${
							user.premium
								? ` (${
										new Date(parseInt(user.premiumSince))
											.toLocaleString()
											.split(',')[0]
								  })`
								: ''
						...
						`No. of mutual servers: \`${
							bot.guilds.cache.filter((g) => g.members.cache.get(user.id)).size
						}\``,
					].join('\n')
				)
		...
		switch (message.args[1].toLowerCase()) {
				...
				try {
						...
						return message.channel
							.error('misc:INCORRECT_FORMAT', {
								EXAMPLE: settings.prefix.concat(
									message.translate('host/user:USAGE')
								),
							})
					...
					if (!resp) {
						...
						await userSchema.findOneAndUpdate(
							{ userID: user.id },
							{ premium: message.args[2], premiumSince: Date.now() }
						)
					}
					user.premium = message.args[2]
					message.channel
						.success('host/user:SUCCESS_PREM')
			...
			case 'rank':
				...
				if (message.attachments.first().url) {
					try {
						...
						if (
							!['png', 'jpeg'].includes(
								response.headers['content-type'].replace('image/', '')
							)
						)
							return message.channel
								.error(
									`File type must be \`PNG\` or \`JPEG\`, this file type was: ${response.headers[
										'content-type'
									].replace('image/', '')}`
								)
								.then((m) => m.timedDelete({ timeout: 5000 }))
						...
						message.channel
							.success('host/user:SUCCESS_RANK')
							.then((m) => m.timedDelete({ timeout: 10000 }))
					} catch (err) {
						if (message.deletable) message.delete()
						bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`)
					...
					return message.channel
						.error('Please upload either a PNG or JPEG file with the command.')
						.then((m) => m.timedDelete({ timeout: 5000 }))
				}
				break
			case 'reset':
				try {
					...
					user.rankImage = ''
					message.channel
						.success('host/user:SUCCESS_RESET')
						.then((m) => m.timedDelete({ timeout: 10000 }))
				} catch (err) {
					if (message.deletable) message.delete()
					bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`)
			...
			default:
				message.channel
					.error('misc:INCORRECT_FORMAT', {
						EXAMPLE: settings.prefix.concat(message.translate('host/user:USAGE')),
					})
					.then((m) => m.timedDelete({ timeout: 5000 }))
				break
		}
