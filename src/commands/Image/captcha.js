		...
		try {
			const json = await fetch(
				encodeURI(
					...
					}&url=${members[0].user.displayAvatarURL({ format: 'png', size: 512 })}`
				)
			).then((res) => res.json())

			// send image
			const embed = new Embed(bot, message.guild).setColor(9807270).setImage(json.message)
		...
		try {
			const json = await fetch(
				encodeURI(
					...
					}&url=${member.user.displayAvatarURL({ format: 'png', size: 512 })}`
				)
			).then((res) => res.json())
