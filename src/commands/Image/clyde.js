...
class Clyde extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!text)
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('image/clyde:USAGE')),
				})
		...
		try {
			...
			// send image
			const embed = new Embed(bot, message.guild).setColor(3447003).setImage(json.message)
	...
	async callback(bot, interaction, guild, args) {
		...
		if (text.length >= 71)
			return interaction.reply({
				embeds: [channel.error('image/clyde:TOO_LONG', {}, true)],
				ephemeral: true,
			})
