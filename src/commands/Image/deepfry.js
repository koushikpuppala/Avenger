...
class Deepfry extends Command {
	...
	async run(bot, message) {
		...
		try {
			const json = await fetch(
				encodeURI(`https://nekobot.xyz/api/imagegen?type=deepfry&image=${files[0]}`)
			).then((res) => res.json())
			// send image
			const embed = new Embed(bot, message.guild).setColor(15105570).setImage(json.message)
	...
	async callback(bot, interaction, guild, args) {
		...
		try {
			const json = await fetch(
				encodeURI(
					`https://nekobot.xyz/api/imagegen?type=deepfry&image=${member.user.displayAvatarURL(
