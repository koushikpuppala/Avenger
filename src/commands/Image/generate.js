...
class Generate extends Command {
	...
	constructor(bot) {
		super(bot, {
			...
			cooldown: 5000,
			examples: [
				'generate 3000years username',
				'generate beautiful <attachment>',
				'generate list',
			],
			slash: true,
			options: [
				{
					...
					choices: [
						...
						...image_1.map((item) => ({ name: item, value: item })),
						...image_2.map((item) => ({ name: item, value: item })),
					].slice(0, 24),
				},
				{
					name: 'user',
	...
	async run(bot, message) {
		...
		if (!message.args[0] || ['list', '?'].includes(message.args[0])) {
			const embed = new Embed(bot, message.guild).setDescription(
				message.translate('image/generate:DESC', {
					IMG_1: `${image_1.join('`, `')}`,
					IMG_2: `${image_2.join('`, `')}`,
				})
			)
		...
		if (image_2.includes(choice) && !files[1])
			return message.channel
				.error('image/generate:NEED_2IMG')
				.then((m) => m.timedDelete({ timeout: 5000 }))

		// send 'waiting' message to show bot has recieved message
		const msg = await message.channel.send(
			message.translate('misc:GENERATING_IMAGE', {
		...
		// get image
		const options = image_1.includes(choice)
			? { url: files[0] }
			: { avatar: files[1], url: files[0] }
		...
		try {
			...
			const attachment = new MessageAttachment(
				image.data,
				`${choice}.${choice == 'triggered' ? 'gif' : 'png'}`
			)
			msg.delete()
	...
	async callback(bot, interaction, guild, args) {
		...
		const member = guild.members.cache
			.get(args.get('user')?.value ?? interaction.user.id)
			.user.displayAvatarURL({ format: 'png', size: 1024 })
		const member2 = guild.members.cache
			.get(args.get('user2')?.value ?? interaction.user.id)
			.user.displayAvatarURL({ format: 'png', size: 1024 })
		...
		try {
			if (option == 'list') {
				const embed = new Embed(bot, guild).setDescription(
					guild.translate('image/generate:DESC', {
						IMG_1: `${image_1.join('`, `')}`,
						IMG_2: `${image_2.join('`, `')}`,
					})
				)
			...
			// generate image
			const options = image_1.includes(option)
				? { url: member }
				: { avatar: member2, url: member }
			...
			const attachment = new MessageAttachment(
				image.data,
				`${option}.${option == 'triggered' ? 'gif' : 'png'}`
			)
