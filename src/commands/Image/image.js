...
class Image extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0]) {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('image/image:USAGE')),
				})
		...
		try {
			const results = await image_search({
				query: message.args.join(' '),
				moderate: message.channel.nsfw || message.channel.type == 'dm' ? false : true,
				iterations: 2,
				retries: 2,
			})

			// send image
			const embed = new Embed(bot, message.guild).setImage(
	...
	async callback(bot, interaction, guild, args) {
		...
		try {
			const results = await image_search({
				query: topic,
				moderate: channel.nsfw || channel.type == 'dm' ? false : true,
				iterations: 2,
				retries: 2,
			})
			const embed = new Embed(bot, guild).setImage(
				results[Math.floor(Math.random() * results.length)].image
			)
