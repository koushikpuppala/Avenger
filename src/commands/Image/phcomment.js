...
class PHcomment extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!text)
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('image/phcomment:USAGE')),
				})
		...
		if (text.length >= 71)
			return message.channel
				.error('image/phcomment:TOO_LONG')
				.then((m) => m.timedDelete({ timeout: 5000 }))
		...
		try {
			const json = await fetch(
				encodeURI(
					...
					}&image=${members[0].user.displayAvatarURL({
						format: 'png',
						size: 512,
					})}&text=${text}`
				)
	...
	async callback(bot, interaction, guild, args) {
		...
		if (text.length >= 71)
			return interaction.reply({
				embeds: [channel.error('image/phcomment:TOO_LONG', {}, true)],
				ephemeral: true,
			})
