...
class QRcode extends Command {
	...
	async run(bot, message) {
		...
		const text = !message.args[0]
			? await message.getImage().then((r) => r[0])
			: message.args.join(' ')
		...
		try {
			...
			// send image in embed
			const embed = new Embed(bot, message.guild).setImage('attachment://QRCODE.png')
	...
	async callback(bot, interaction, guild, args) {
		...
		try {
			...
			const embed = new Embed(bot, guild).setImage('attachment://QRCODE.png')
