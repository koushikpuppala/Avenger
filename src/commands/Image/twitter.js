...
class Twitter extends Command {
	...
	async run(bot, message, settings) {
		...
		if (message.args.join(' ').replace(/<@.?[0-9]*?>/g, '').length == message.args.length)
			message.args.shift()
		...
		if (message.args.length == 0)
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('image/twitter:USAGE')),
				})
		...
		try {
			const json = await fetch(
				encodeURI(
					`https://nekobot.xyz/api/imagegen?type=tweet&username=${members[0].user.username}&text=${text}`
				)
			).then((res) => res.json())

			// send image in embed
			const embed = new Embed(bot, message.guild).setImage(json.message)
	...
	async callback(bot, interaction, guild, args) {
		...
		if (text.length >= 61)
			return interaction.reply({
				embeds: [channel.error('image/twitter:TOO_LONG', {}, true)],
				ephemeral: true,
			})
		...
		try {
			const json = await fetch(
				encodeURI(
					`https://nekobot.xyz/api/imagegen?type=tweet&username=${member.user.username}&text=${text}`
				)
			).then((res) => res.json())
			const embed = new Embed(bot, guild).setColor(3447003).setImage(json.message)
