...
const ordinal = (num) =>
	`${num.toLocaleString('en-US')}${[, 'st', 'nd', 'rd'][(num / 10) % 10 ^ 1 && num % 10] || 'th'}`
...
class Leaderboard extends Command {
	...
	async createLeaderboard(bot, guild) {
		...
		if (!res[0]) {
			// If there no results
			embed.addField(
				guild.translate('level/leaderboard:EMPTY_TITLE'),
				guild.translate('level/leaderboard:EMPTY_DESC')
			)
			return embed
		} else {
			...
			for (let i = 0; i < pagesNum; i++) {
				...
				for (let j = 0; j < 10; j++) {
					if (res[i * 10 + j]) {
						...
						const name =
							(await guild.members.fetch(res[i * 10 + j].userID).catch(() => {})) ||
							'User left'
						if (name == 'User left') {
							embed2.addField(
								guild.translate('level/leaderboard:FIELD_TITLE', {
									POS: ordinal(i * 10 + j + 1),
									NAME: name,
								}),
								guild.translate('level/leaderboard:FIELD_DATA', {
									XP: res[i * 10 + j].Xp.toLocaleString(guild.settings.Language),
									LEVEL: res[i * 10 + j].Level.toLocaleString(
