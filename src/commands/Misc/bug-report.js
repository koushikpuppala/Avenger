...
module.exports = class BugReport extends Command {
	...
	async run(bot, message, settings) {
		...
		const embed = new MessageEmbed()
			.setAuthor(
				`"${message.author.tag}" reported a bug`,
		...
		return message.channel.send(
			'<a:tick:818394472563605554>Your bug report has been sent to the support server. You will hear back from my owner in DMs if there is anything wrong with your report. Have a nice day!'
		)
	}
}
