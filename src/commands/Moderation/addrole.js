		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/addrole:USAGE')),
				})
		...
		fs.readFile('./src/assets/json/colours.json', async (err, data) => {
			...
			if (colourNames[colour] ?? /[0-9A-Fa-f]{6}/g.test(message.args[1])) {
				const role = await message.guild.roles.create({
					name: message.args[0],
					reason: 'Created a new role with the bot',
					color: colourNames[colour] ?? message.args[1],
					hoist: message.args[2] ?? false,
				})
				message.channel
					.success('moderation/addrole:SUCCESS', { ROLE: role.id })
					.then((m) => m.timedDelete({ timeout: 5000 }))
			} else {
				const role = await message.guild.roles.create({
					name: message.args[0],
