...
class Ban extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/ban:USAGE')),
				})
				.then((m) => m.timedDelete({ timeout: 10000 }))

		// Get user and reason
		const reason = message.args[1]
			? message.args.splice(1, message.args.length).join(' ')
			: message.translate('misc:NO_REASON')
		...
		if (!members[0])
			return message.channel
				.error('moderation/ban:MISSING_USER')
				.then((m) => m.timedDelete({ timeout: 10000 }))

		// Make sure user isn't trying to punish themselves
		if (members[0].user.id == message.author.id)
			return message.channel
				.error('misc:SELF_PUNISH')
				.then((m) => m.timedDelete({ timeout: 10000 }))

		// Make sure user does not have ADMINISTRATOR permissions or has a higher role
		if (
			members[0].permissions.has('ADMINISTRATOR') ||
			members[0].roles.highest.comparePositionTo(message.guild.me.roles.highest) >= 0
		) {
			return message.channel
				.error('moderation/ban:TOO_POWERFUL')
				.then((m) => m.timedDelete({ timeout: 10000 }))
		}
		...
		try {
			...
			try {
				const embed = new Embed(bot, message.guild)
					...
					.setDescription(
						message.translate('moderation/ban:DESC', { NAME: message.guild.name })
					)
			...
			message.channel
				.success('moderation/ban:SUCCESS', { USER: members[0].user })
				.then((m) => m.timedDelete({ timeout: 8000 }))

			// Check to see if this ban is a tempban
			const possibleTime = message.args[message.args.length - 1]
			if (
				possibleTime.endsWith('d') ||
				possibleTime.endsWith('h') ||
				possibleTime.endsWith('m') ||
				possibleTime.endsWith('s')
			) {
