	...
	async run(bot, message, settings) {
		...
		if (!amount)
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/clear:USAGE')),
				})
		...
		if (amount >= 100) {
			...
			const row = new MessageActionRow()
				.addComponents(
					new MessageButton()
						...
						.setStyle('SUCCESS')
						.setEmoji(
							message.channel.checkPerm('USE_EXTERNAL_EMOJIS')
								? bot.customEmojis['checkmark']
								: '✅'
						)
				)
				.addComponents(
			...
			await message.channel.send({ embeds: [embed], components: [row] }).then(async (msg) => {
				// create collector
				const filter = (i) =>
					['cancel', 'success'].includes(i.customId) && i.user.id === message.author.id
				...
				collector.on('collect', async (i) => {
					...
					if (i.customId === 'cancel') {
						...
						await message.channel.send(
							message.translate('moderation/clear:DEL_MSG', {
								TIME: Math.ceil(amount / 100) * 5,
								NUM: amount,
							})
						)
							...
							try {
								...
								if (message.args[1]) {
									...
									messages = messages.filter(
										(m) => m.author.id == member[0].user.id
									)
								}

								// delete the message
								const delMessages = await message.channel
									.bulkDelete(messages, true)
									.catch((err) =>
										bot.logger.error(
											`Command: '${this.help.name}' has error: ${err.message}.`
										)
									)
								y += delMessages.size
								x++
						...
						return message.channel
							.success('moderation/clear:SUCCESS', { NUM: y })
			...
			await message.channel.messages.fetch({ limit: amount }).then(async (messages) => {
				...
				message.channel
					.success('moderation/clear:SUCCESS', { NUM: messages.size })
					.then((m) => m.timedDelete({ timeout: 3000 }))
			})
		}
	}
