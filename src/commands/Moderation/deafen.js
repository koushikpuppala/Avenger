	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/deafen:USAGE')),
				})
		...
		if (members[0]?.voice.channel) {
			...
			if (!members[0].voice.channel.permissionsFor(bot.user).has('DEAFEN_MEMBERS')) {
				...
				return message.channel
					.error('misc:MISSING_PERMISSION', {
						PERMISSIONS: message.translate('permissions:DEAFEN_MEMBERS'),
					})
					.then((m) => m.timedDelete({ timeout: 10000 }))
			}
			...
			try {
				...
				message.channel
					.success('moderation/deafen:SUCCESS', { USER: members[0].user })
