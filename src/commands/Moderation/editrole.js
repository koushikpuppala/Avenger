...
class EditRole extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/editrole:USAGE')),
				})
		...
		if (!role[0])
			return message.channel
				.error('moderation/delrole:MISSING')
				.then((m) => m.timedDelete({ timeout: 5000 }))

		if (
			message.member.permissions.has('ADMINISTRATOR') ||
			role[0].comparePositionTo(message.guild.me.roles.highest) >= 0
		) {
			switch (message.args[1].toLowerCase()) {
				...
				case 'color':
					fs.readFile('./src/assets/json/colours.json', async (err, data) => {
						...
						if (colourNames[colour] ?? /[0-9A-Fa-f]{6}/g.test(message.args[2])) {
							...
							message.channel
								.success('moderation/editrole:SUCCESS')
								.then((m) => m.timedDelete({ timeout: 3000 }))
						} else {
							return message.channel
								.error('misc:ERROR_MESSAGE', { ERROR: 'Colour not found' })
								.then((m) => m.timedDelete({ timeout: 5000 }))
						}
					})
					break
				case 'hoist':
					if (!['true', 'false'].includes(message.args[2]))
						return message.channel
							.error('misc:ERROR_MESSAGE', {
								ERROR: 'Please provide the boolean as a value',
							})
							.then((m) => m.timedDelete({ timeout: 5000 }))
				...
				case 'rename':
					if (message.args[2].length >= 100)
						return message.channel
							.error('misc:ERROR_MESSAGE', {
								ERROR: 'The role name is greater than the character limit of (100)',
							})
							.then((m) => m.timedDelete({ timeout: 5000 }))
					...
					message.channel
						.error('misc:INCORRECT_FORMAT', {
							EXAMPLE: settings.prefix.concat(
								message.translate('moderation/editrole:USAGE')
							),
						})
						.then((m) => m.timedDelete({ timeout: 5000 }))
			}
