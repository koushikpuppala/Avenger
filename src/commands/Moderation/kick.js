...
class Kick extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/kick:USAGE')),
				})
		...
		if (
			...
			return message.channel
				.error('moderation/kick:TOO_POWERFUL')
				.then((m) => m.timedDelete({ timeout: 10000 }))
		}
		...
		try {
			...
			try {
				const embed = new Embed(bot, message.guild)
					...
					.setDescription(
						message.translate('moderation/kick:DESC', { NAME: message.guild.name })
					)
			...
			message.channel
				.success('moderation/kick:SUCCESS', { USER: members[0].user })
