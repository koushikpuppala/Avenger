		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/mute:USAGE')),
				})
			...
			if (!channel.permissionsFor(bot.user).has('MUTE_MEMBERS')) {
				...
				return message.channel
					.error('misc:MISSING_PERMISSION', {
						PERMISSIONS: message.translate('permissions:MUTE_MEMBERS'),
					})
					.then((m) => m.timedDelete({ timeout: 10000 }))
			}
		}
		...
		try {
			members[0].roles.add(muteRole).then(async () => {
				...
				if (!settings.MutedMembers.includes(members[0].user.id)) {
					await message.guild.updateGuild({
						MutedMembers: [...settings.MutedMembers, members[0].user.id],
					})
				}

				// reply to user
				message.channel
					.success('moderation/mute:SUCCESS', { USER: members[0].user })
