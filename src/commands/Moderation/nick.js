...
class Nick extends Command {
	...
	async run(bot, message, settings) {
		...
		if (
			...
			return message.channel
				.error('moderation/nick:TOO_POWERFUL')
				.then((m) => m.timedDelete({ timeout: 10000 }))
		}
		...
		if (message.args.length == 0)
			return message.channel
				.error('moderation/nick:ENTER_NICKNAME')
				.then((m) => m.timedDelete({ timeout: 10000 }))
		...
		if (nickname.length >= 32)
			return message.channel
				.error('moderation/nick:LONG_NICKNAME')
		...
		try {
			...
			message.channel
				.success('moderation/nick:SUCCESS', { USER: members[0].user })
				.then((m) => m.timedDelete({ timeout: 5000 }))
		} catch (err) {
