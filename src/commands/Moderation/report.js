	...
	async run(bot, message, settings) {
		...
		if (settings.ModLogEvents?.includes('REPORT')) {
				...
				return message.channel
					.error('misc:INCORRECT_FORMAT', {
						EXAMPLE: settings.prefix.concat(
							message.translate('moderation/report:USAGE')
						),
					})
			...
			const embed = new Embed(bot, message.guild)
				.setAuthor(
					message.translate('moderation/report:AUTHOR'),
					members[0].user.displayAvatarURL
				)
				.setColor(15158332)
				...
				.addField(
					message.translate('moderation/report:REASON'),
					message.args.slice(1).join(' ')
				)
				.setTimestamp()
				.setFooter(message.guild.name)
			const repChannel = message.guild.channels.cache.find(
				(channel) => channel.id === settings.ModLogChannel
			)
			if (repChannel) {
				repChannel.send({ embeds: [embed] })
				message.channel
					.success('moderation/report:SUCCESS', { USER: members[0].user })
					.then((m) => m.timedDelete({ timeout: 3000 }))
			}
		} else {
			message.channel
				.error('misc:ERROR_MESSAGE', { ERROR: 'Logging: `REPORTS` has not been setup' })
				.then((m) => m.timedDelete({ timeout: 5000 }))
		}
	}
}
