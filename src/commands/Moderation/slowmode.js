...
// Dependencies
const {
		time: { getTotalTime, getReadableTime },
	} = require('../../utils'),
...
class Slowmode extends Command {
	...
	async run(bot, message, settings) {
		...
		if (message.args[0] == 'off') {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('moderation/slowmode:USAGE')),
				})
		...
		try {
			...
			message.channel
				.success('moderation/slowmode:SUCCESS', {
					TIME: time == 0 ? message.translate('misc:OFF') : getReadableTime(time),
				})
