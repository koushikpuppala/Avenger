			...
			await WarningSchema.find(
				...
				(err, warn) => {
					...
					if (!warn[0]) {
						...
						message.channel
							.error('moderation/warnings:NO_WARNINGS')
							.then((m) => m.timedDelete({ timeout: 3500 }))
					} else {
						// Warnings have been found
						let list = `Warnings (${warn.length}):\n`
						for (let i = 0; i < warn.length; i++) {
							list += `${i + 1}.) ${warn[i].Reason} | ${
								message.guild.members.cache.get(warn[i].Moderater)
									? message.guild.members.cache.get(warn[i].Moderater)
									: 'User left'
							} (Issue date: ${warn[i].IssueDate})\n`
						}

						const embed = new Embed(bot, message.guild)
							.setTitle('moderation/warnings:TITLE', {
								USER: members[0].user.username,
							})
							.setDescription(list)
							.setTimestamp()
