		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('music/p-add:USAGE')),
				})
				...
				if (p) {
					...
					if (res.loadType == 'NO_MATCHES') {
						...
						try {
							...
							const newTracks = res.tracks.slice(
								0,
								(message.author.premium ? 200 : 100) - p.songs.length
							)
							p.songs.push(...newTracks)
							p.duration =
								parseInt(p.duration) +
								parseInt(res.tracks.reduce((prev, curr) => prev + curr.duration, 0))
							await p.save()
							message.channel.success('music/p-add:SUCCESS', {
								NUM: newTracks.length,
						...
						const embed = new Embed(bot, message.guild)
							...
							.setDescription(
								message.translate('music/search:DESC', { RESULTS: results })
							)
						...
						if (index < 0 || index > max - 1)
							return message.reply(
								message.translate('music/search:INVALID', { NUM: max })
							)
