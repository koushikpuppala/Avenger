...
class PCreate extends Command {
	...
	constructor(bot) {
		super(bot, {
			...
			cooldown: 3000,
			examples: [
				'p-create Songs https://www.youtube.com/watch?v=N3vY6yvHLdc&list=PLUhFQlEQDZOfDqu5eZUvwUs7EEcPpeVbB',
			],
		})
	}
		...
		if (!message.args[1])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('music/p-create:USAGE')),
				})
				.then((m) => m.timedDelete({ timeout: 5000 }))
		if (message.args[0].length > 32)
			return msg.edit(message.translate('music/p-create:TOO_LONG'))
	...
	async savePlaylist(bot, message, settings, msg) {
		...
		try {
			...
			return message.channel
				.error('music/play:ERROR', { ERROR: err.message })
				.then((m) => m.timedDelete({ timeout: 5000 }))
		}
		...
		if (res.loadType == 'NO_MATCHES') {
			...
			if (res.loadType == 'SEARCH_RESULT') {
				...
				if (index < 0 || index > max - 1)
					return message.reply(message.translate('music/search:INVALID', { NUM: max }))
			...
			const embed = new Embed(bot, message.guild)
				...
				.setDescription(
					[
						...
						message.translate('music/p-create:DESC_2', {
							NUM: getReadableTime(parseInt(newPlaylist.duration)),
						}),
						message.translate('music/p-create:DESC_3', {
							NAME:
								res.loadType == 'PLAYLIST_LOADED'
									? res.playlist.name
									: tracks[0].title,
				...
				.setFooter('music/p-create:FOOTER', {
					ID: newPlaylist._id,
					NUM: newPlaylist.songs.length,
					PREM: message.author.premium ? '200' : '100',
				})
				.setTimestamp()
