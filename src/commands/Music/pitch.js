...
class Pitch extends Command {
	...
	async run(bot, message) {
		...
		if (
			message.args[0] &&
			(message.args[0].toLowerCase() == 'reset' || message.args[0].toLowerCase() == 'off')
		) {
			player.resetFilter()
			const msg = await message.channel.send(message.translate('music/pitch:PITCH_OFF'))
			const embed = new MessageEmbed().setDescription(message.translate('music/pitch:DESC_1'))
			await bot.delay(5000)
			return msg.edit({ content: '​​ ', embeds: [embed] })
		}

		if (isNaN(message.args[0]))
			return message.channel.send(message.translate('music/pitch:INVALID'))
		if (message.args[0] < 0 || message.args[0] > 10)
			return message.channel.send(message.translate('music/pitch:INCORRECT'))

		player.setFilter({
			timescale: { pitch: message.args[0] },
		})
		const msg = await message.channel.send(
			message.translate('music/pitch:PITCH_ON', { NUM: message.args[0] })
		)
		const embed = new MessageEmbed().setDescription(
			message.translate('music/pitch:DESC_2', { NUM: message.args[0] })
		)
		await bot.delay(5000)
		return msg.edit({ content: '​​ ', embeds: [embed] })
	}
	...
	async callback(bot, interaction, guild, args) {
		...
		if (amount && ['reset', 'off'].includes(amount.toLowerCase())) {
			...
			const embed = new MessageEmbed(bot, guild).setDescription(
				bot.translate('music/pitch:DESC_1')
			)
			await bot.delay(5000)
			...
			const embed = new MessageEmbed(bot, guild).setDescription(
				bot.translate('music/pitch:DESC_2', { NUM: amount })
			)
			await bot.delay(5000)
			return interaction.editReply({ content: '​​ ', embeds: [embed] })
		}
