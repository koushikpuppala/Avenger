	...
	async run(bot, message, settings) {
		...
		if (message.args.length == 0) {
			...
			if (message.attachments.size > 0) {
				...
				if (!message.args[0])
					return message.channel
						.error('music/play:INVALID_FILE')
						.then((m) => m.timedDelete({ timeout: 10000 }))
			} else {
				return message.channel
					.error('music/play:NO_INPUT')
	...
	async callback(bot, interaction, guild, args) {
		...
		if (bot.manager?.players.get(guild.id)) {
			if (member.voice.channel.id != bot.manager?.players.get(guild.id).voiceChannel)
				return interaction.reply({
					ephemeral: true,
					embeds: [channel.error('misc:NOT_VOICE', { ERROR: null }, true)],
				})
		}
		...
		if (res.loadType == 'NO_MATCHES') {
			...
			return interaction.reply({
				ephemeral: true,
				embeds: [channel.error('music/play:NO_SONG', { ERROR: null }, true)],
			})
		} else if (res.loadType == 'PLAYLIST_LOADED') {
			// Connect to voice channel if not already
			if (player.state !== 'CONNECTED') player.connect()
