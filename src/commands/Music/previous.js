...
class Previous extends Command {
	...
	async run(bot, message, settings) {
		...
		if (queue.size == 0) {
			const embed = new Embed(bot, message.guild).setTitle(
				'No previous tracks have been played'
			)
		...
		if (!message.args[0]) {
			if (pages.length == pagesNum && player.previousTracks.length > 10)
			...
			if (message.args[0] > pagesNum)
				return message.channel.send(`There are only ${pagesNum} pages available.`)
