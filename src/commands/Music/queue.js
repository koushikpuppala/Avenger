...
class Queue extends Command {
	...
	async run(bot, message, settings) {
		...
		if (queue.size == 0) {
			const embed = new Embed(bot, message.guild).setTitle('music/queue:EMPTY')
		...
		const parsedQueueDuration = getReadableTime(
		...
		for (let i = 0; i < player.queue.length; i++) {
			...
			songStrings.push(
				`**${i + 1}.** [${song.title}](${song.uri}) \`[${getReadableTime(
					song.duration
		...
		if (!message.args[0]) {
			if (pages.length == pagesNum && player.queue.length > 10)
				paginate(bot, message.channel, pages)
			else return message.channel.send({ embeds: [pages[0]] })
		} else {
			if (isNaN(message.args[0]))
				return message.channel.send(message.translate('music/queue:NAN'))
			if (message.args[0] > pagesNum)
				return message.channel.send(
					message.translate('music/queue:TOO_HIGH', { NUM: pagesNum })
				)
	...
	async callback(bot, interaction, guild, args) {
		...
		if (guild.roles.cache.get(guild.settings.MusicDJRole)) {
			if (!member.roles.cache.has(guild.settings.MusicDJRole)) {
				return interaction.reply({
					ephemeral: true,
					embeds: [channel.error('misc:MISSING_ROLE', { ERROR: null }, true)],
				})
			}
		}
		...
		if (!player)
			return interaction.reply({
				ephemeral: true,
				embeds: [channel.error('misc:NO_QUEUE', { ERROR: null }, true)],
			})

		// Make sure queue is not empty
		const queue = player.queue
		if (queue.size == 0) {
			const embed = new Embed(bot, guild).setTitle('music/queue:EMPTY')
		...
		if (!page) {
			...
			if (page > pagesNum)
				return interaction.reply({
					ephemeral: true,
					embeds: [channel.error('music/queue:TOO_HIGH', { NUM: pagesNum }, true)],
				})
