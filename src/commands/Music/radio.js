	...
	async run(bot, message, settings) {
		...
		if (bot.manager?.players.get(message.guild.id)) {
			if (
				...
				return message.channel
					.error('misc:NOT_VOICE')
					.then((m) => m.timedDelete({ timeout: 10000 }))
		}

		// Check if VC is full and bot can't join doesn't have (MANAGE_CHANNELS)
		if (
			message.member.voice.channel.full &&
			!message.member.voice.channel.permissionsFor(message.guild.me).has('MOVE_MEMBERS')
		) {
			return message.channel
				.error('music/play:VC_FULL')
				.then((m) => m.timedDelete({ timeout: 10000 }))
		}
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('music/radio:USAGE')),
				})
		...
		await getStations({
			...
			if (!data[0])
				return message.channel
					.send('No radio found with that name')
					.then((m) => m.timedDelete({ timeout: 10000 }))
			...
			const filter = (m) =>
				m.author.id === message.author.id && /^(\d+|cancel)$/i.test(m.content)
			...
			if (index < 0 || index > max - 1)
				return message.reply(`The number you provided was too small or too big (1-${max}).`)
	...
	async callback(bot, interaction, guild, args) {
		...
		if (guild.roles.cache.get(guild.settings.MusicDJRole)) {
			if (!member.roles.cache.has(guild.settings.MusicDJRole)) {
				interaction.reply({
					embeds: [channel.error('misc:MISSING_ROLE', {}, true)],
					ephemeral: true,
				})
			}
		}
		...
		if (!member.voice.channel)
			return interaction.reply({
				embeds: [channel.error('music/play:NOT_VC', {}, true)],
				ephemeral: true,
			})

		// Check that user is in the same voice channel
		if (bot.manager?.players.get(guild.id)) {
			if (member.voice.channel.id != bot.manager?.players.get(guild.id).voiceChannel)
				return interaction.reply({
					embeds: [channel.error('misc:NOT_VOICE', {}, true)],
					ephemeral: true,
				})
		}
		...
		if (
			...
			!member.voice.channel.permissionsFor(guild.me).has('MOVE_MEMBERS')
		) {
			return interaction.reply({
				embeds: [channel.error('music/play:VC_FULL', {}, true)],
				ephemeral: true,
			})
		}
		...
		await getStations({
			...
			language: bot.languages.find((l) => l.name.includes(guild.settings.Language))
				.nativeName,
			by: 'name',
			...
			if (!data[0])
				return interaction.reply({
					embeds: [channel.error('music/radio:MISSING_RADIO', {}, true)],
					ephemeral: true,
				})
			...
			const filter = (m) =>
				m.author.id === member.user.id && /^(\d+|cancel)$/i.test(m.content)
			const max = data.length
			...
			try {
				...
				return interaction.reply({
					embeds: [channel.error('misc:WAITED_TOO_LONG', {}, true)],
					ephemeral: true,
				})
			}
			...
			if (first.toLowerCase() === 'cancel') {
				...
				return interaction.editReply({
					embeds: [channel.error('misc:CANCELLED', {}, true)],
					ephemeral: true,
				})
			}
			...
			if (index < 0 || index > max - 1)
				return interaction.editReply({
					embeds: [
						channel.error(
							`The number you provided was too small or too big (1-${max}).`,
							{},
							true
						),
					],
			...
			if (res.loadType == 'NO_MATCHES') {
				...
				if (!player.queue.current) player.destroy()
				return interaction.editReply({
					embeds: [channel.error('music/play:NO_SONG', {}, true)],
					ephemeral: true,
				})
			} else {
				// add track to queue and play
