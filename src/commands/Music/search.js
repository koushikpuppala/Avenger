...
class Search extends Command {
	...
	async run(bot, message, settings) {
		...
		try {
			...
			return message.channel
				.error('music/search:ERROR', { ERROR: err.message })
				.then((m) => m.timedDelete({ timeout: 5000 }))
		}
		...
		if (res.loadType == 'NO_MATCHES') {
			...
			if (!player.playing && !player.paused && !player.queue.size) {
				...
				message.channel.send(
					message.translate('music/search:ADDED', { TITLE: track.title })
				)
			}
		}
	}
	...
	async callback(bot, interaction, guild, args) {
		...
		if (res.loadType == 'NO_MATCHES') {
			...
			if (index < 0 || index > max - 1)
				return interaction.reply({
					content: guild.translate('music/search:INVALID', { NUM: max }),
				})
			...
			if (!player.playing && !player.paused && !player.queue.size) {
				...
				interaction.reply({
					content: guild.translate('music/search:ADDED', { TITLE: track.title }),
				})
			}
		}
	}
