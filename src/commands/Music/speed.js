...
class Speed extends Command {
	...
	async run(bot, message) {
		...
		if (!player.queue.current.isSeekable)
			return message.channel
				.error('music/speed:LIVESTREAM')
				.then((m) => m.timedDelete({ timeout: 10000 }))

		// Make sure Number is a number
		if (isNaN(message.args[0]) || message.args[0] < 0 || message.args[0] > 10)
			return message.channel
				.error('music/speed:INVALID')
				.then((m) => m.timedDelete({ timeout: 10000 }))
		...
		try {
			...
			const embed = new Embed(bot, message.guild).setDescription(
				message.translate('music/speed:UPDATED', { NUM: player.speed })
			)
	...
	async callback(bot, interaction, guild, args) {
		...
		if (!player.queue.current.isSeekable)
			return interaction.reply({
				ephemeral: true,
				embeds: [channel.error('music/speed:LIVESTREAM', { ERROR: null }, true)],
			})
		...
		try {
			...
			const embed = new Embed(bot, guild).setDescription(
				guild.translate('music/speed:UPDATED', { NUM: player.speed })
			)
			await bot.delay(5000)
			player.speed = speed
