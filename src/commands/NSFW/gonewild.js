...
class Gonewild extends Command {
	...
	async run(bot, message) {
		...
		const msg = await message.channel.send(
			message.translate('nsfw/4k:FETCHING', {
		...
		try {
			get('https://nekobot.xyz/api/image?type=gonewild').then((res) => {
				msg.delete()
				const embed = new Embed(bot, message.guild).setImage(res.data.message)
	...
	async callback(bot, interaction, guild) {
		...
		try {
			get('https://nekobot.xyz/api/image?type=gonewild').then((res) => {
				const embed = new Embed(bot, guild).setImage(res.data.message)
