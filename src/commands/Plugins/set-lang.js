...
class Setlang extends Command {
	...
	async run(bot, message, settings) {
		...
		// get language
		if (message.args[0]) {
			const language = bot.languages.find(
				(l) => l.name === message.args[0] || l.aliases.includes(message.args[0])
			)
			if (!message.args[0] || !language) {
				return message.channel.error('plugins/set-lang:MISSING_LANG', {
					list: bot.languages.map((l) => '`' + l.name + '`').join(', '),
				})
			} else {
			...
			const row = new MessageActionRow().addComponents(
				new MessageSelectMenu()
					.setCustomId('setlang')
					.setPlaceholder(
						bot.languages.find((lan) => lan.name == settings.Language).nativeName
					)
					.addOptions(options)
			)
			...
			// send message
			message.channel
				.send({ content: 'Please select a language for this server', components: [row] })
				.then(async (msg) => {
					const filter = (interaction) =>
						interaction.customId === 'setlang' &&
					...
					try {
						...
						await updateLanguage(
							message.guild,
							bot.languages.find((l) => l.name === resp.values[0])
						)
						msg.delete()
					} catch {
						msg.delete()
		...
		async function updateLanguage(guild, language) {
			try {
				...
				return message.channel.success('plugins/set-lang:SUCCESS', {
