	...
	async run(bot, message, settings) {
		...
		if (message.args[0] == 'true' || message.args[0] == 'false') {
			...
			try {
				...
				message.channel
					.success('plugins/set-logs:TOGGLE', { TOGGLE: message.args[0] })
			...
			if (!message.args[1] || !features.includes(message.args[1].toUpperCase())) {
				...
				const embed = new Embed(bot, message.guild)
					...
					.setDescription(
						message.translate('plugins/set-logs:DESC', {
							FEAT: features.join('`, `'),
							CUR_FEAT: currentFeatures.join('`, `'),
						})
					)
				message.channel
					.send({ embeds: [embed] })
				...
				try {
					...
					const events = message.args
						.map((args) => args.toUpperCase())
						.filter((arg) => currentFeatures.indexOf(arg.toUpperCase()) == -1)
					currentFeatures.push(...events)
					await message.guild.updateGuild({ ModLogEvents: currentFeatures })
					message.channel.success('plugins/set-logs:ADD_LOG', {
						LOG: `\`${events[0] ? events.join('`, `') : 'Nothing'}\``,
					})
				} catch (err) {
					bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`)
					message.channel
					...
					for (const featues of message.args.map((args) => args.toUpperCase())) {
						if (currentFeatures.indexOf(featues.toUpperCase()) > -1)
							currentFeatures.splice(
								currentFeatures.indexOf(featues.toUpperCase()),
								1
							)
					}
			...
			const embed = new Embed(bot, message.guild)
				...
				.setDescription(
					message.translate('plugins/set-logs:DESC_2', {
						ID: settings.ModLogChannel,
						TOGGLE: settings.ModLog,
						FEAT: settings.ModLogEvents.join('`, `'),
					})
				)
