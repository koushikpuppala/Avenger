		...
		await ReactionRoleSchema.find(
			...
			async (err, reacts) => {
				...
				if (reacts.length >= 3 && !message.guild.premium) {
					...
					// You have reached max amount of reaction roles in this server
					...
					const patt =
						/https?:\/\/(?:(?:canary|ptb|www)\.)?discord(?:app)?\.com\/channels\/(?:@me|(?<g>\d+))\/(?<c>\d+)\/(?<m>\d+)/g
					let channel, msgLink
					if (patt.test(message.args[0])) {
						console.log('test')
						const stuff = message.args[0].split('/')
						msgLink = await bot.guilds.cache
							.get(stuff[4])
							?.channels.cache.get(stuff[5])
							?.messages.fetch(stuff[6])
						if (!msgLink) return message.channel.error('Incorrect message link.')
						channel = msgLink.channel
					} else {
						channel =
							message.guild.channels.cache.get(message.args[0]) ?? message.channel
					}
					...
					if (
						...
						return message.channel
							.error('misc:MISSING_PERMISSION', {
								PERMISSIONS: message.translate('permissions:SEND_MESSAGES'),
							})
							.then((m) => m.timedDelete({ timeout: 10000 }))
					} else if (!channel.permissionsFor(bot.user).has('EMBED_LINKS')) {
						return message.channel
							.error('misc:MISSING_PERMISSION', {
								PERMISSIONS: message.translate('permissions:EMBED_LINKS'),
							})
							.then((m) => m.timedDelete({ timeout: 10000 }))
					} else if (!channel.permissionsFor(bot.user).has('ADD_REACTIONS')) {
						return message.channel
							.error('misc:MISSING_PERMISSION', {
								PERMISSIONS: message.translate('permissions:ADD_REACTIONS'),
							})
							.then((m) => m.timedDelete({ timeout: 10000 }))
					}
					...
					const filter = (m) =>
						...
						(m.content == 'cancel' && m.author.id == message.author.id)
					...
					roles = roles.filter((r) => {
						return !(
							r.comparePositionTo(message.guild.me.roles.highest) >= 0 || r.managed
						)
					})
					...
					if (!emojis[0] || emojis.length < roles.length)
						return message.channel.send(
							message.translate('plugins/rr-add:INCORRECT_EMOJIS')
						)
