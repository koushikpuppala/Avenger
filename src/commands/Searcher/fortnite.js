...
class Fortnite extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!['kbm', 'gamepad', 'touch'].includes(message.args[0]))
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/fortnite:USAGE')),
				})
	...
	async createEmbed(bot, guild, username, platform) {
		const data = await new (require('../../APIs/fortnite.js'))(
			bot.config.api_keys.fortnite
		...
		return new Embed(bot, guild)
			...
			.setURL(data.url)
			.setDescription(
				guild.translate('searcher/fortnite:DESC', {
					TOP_3: data.stats.lifetime.top_3.toLocaleString(guild.settings.Language),
					TOP_5: data.stats.lifetime.top_5.toLocaleString(guild.settings.Language),
					TOP_6: data.stats.lifetime.top_6.toLocaleString(guild.settings.Language),
					TOP_12: data.stats.lifetime.top_12.toLocaleString(guild.settings.Language),
					TOP_25: data.stats.lifetime.top_25.toLocaleString(guild.settings.Language),
				})
			)
			.setThumbnail(
			...
			.addField(
				guild.translate('searcher/fortnite:TOTAL'),
				(
					data.stats.solo.score +
					data.stats.duo.score +
					data.stats.squad.score
				).toLocaleString(guild.settings.Language),
				true
			)
			.addField(
				guild.translate('searcher/fortnite:PLAYED'),
				data.stats.lifetime.matches.toLocaleString(guild.settings.Language),
				true
			)
			.addField(
				guild.translate('searcher/fortnite:WINS'),
				data.stats.lifetime.wins.toLocaleString(guild.settings.Language),
				true
			)
			.addField(
				guild.translate('searcher/fortnite:WINS_PRE'),
				`${((data.stats.lifetime.wins / data.stats.lifetime.matches) * 100).toFixed(2)}%`,
				true
			)
			.addField(
				guild.translate('searcher/fortnite:KILLS'),
				`${data.stats.lifetime.kills.toLocaleString(guild.settings.Language)}`,
				true
			)
