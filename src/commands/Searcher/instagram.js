...
class Instagram extends Command {
		...
		if (!username)
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/instagram:USAGE')),
				})
	...
	async createEmbed(bot, username, guild) {
		...
		if (res.size == 0 || !res.graphql?.user.username)
			return guild.translate('searcher/instagram:UNKNOWN_USER')
		...
		return new Embed(bot, guild)
			...
			.addField(
				guild.translate('searcher/instagram:BIOGRAPHY'),
				account.biography.length == 0 ? 'None' : account.biography
			)
			.addField(
				guild.translate('searcher/instagram:POSTS'),
				account.edge_owner_to_timeline_media.count.toLocaleString(guild.settings.Language),
				true
			)
			.addField(
				guild.translate('searcher/instagram:FOLLOWERS'),
				account.edge_followed_by.count.toLocaleString(guild.settings.Language),
				true
			)
			.addField(
				guild.translate('searcher/instagram:FOLLOWING'),
				account.edge_follow.count.toLocaleString(guild.settings.Language),
				true
			)
			.addField(
				guild.translate('searcher/instagram:PRIVATE'),
				account.is_private ? 'Yes 🔒' : 'No 🔓',
				true
			)
			.addField(
				guild.translate('searcher/instagram:VERIFIED'),
