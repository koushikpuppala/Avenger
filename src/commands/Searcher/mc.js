...
class Minecraft extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/mc:USAGE')),
				})
				.then((m) => m.timedDelete({ timeout: 5000 }))

		// send 'waiting' message to show bot has recieved message
		const msg = await message.channel.send(
			message.translate('searcher/mc:FETCHING', {
		...
		// Ping server
		const resp = await this.createEmbed(
			bot,
			message.guild,
			message.channel,
			message.args[0],
	...
	async createEmbed(bot, guild, channel, IP, port) {
		try {
			...
			const embed = new Embed(bot, guild).setColor(0x0099ff).setTitle('searcher/mc:TITLE')
			...
			embed.addField(
				guild.translate('searcher/mc:DESC'),
				response.description.descriptionText.replace(/§[a-zA-Z0-9]/g, '')
			)
			embed.addField(
				guild.translate('searcher/mc:PLAYERS'),
				`${response.onlinePlayers.toLocaleString(
					guild.settings.Language
				)}/${response.maxPlayers.toLocaleString(guild.settings.Language)}`
			)
			if (response.favicon) {
