...
const { Embed } = require('../../utils'),
	...
	{ findByUsername, getProgression, getRanks, getStats } = new R6API({
		email: config.api_keys.rainbow.email,
		password: config.api_keys.rainbow.password,
	}),
...
class Rainbow6Siege extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0]) {
			...
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/r6:USAGE')),
				})
		...
		const msg = await message.channel.send(
			message.translate('searcher/fortnite:FETCHING', {
		...
		// display stats
		const resp = await this.fetchUserData(
			bot,
			message.guild,
			message.channel,
			player,
	...
	async callback(bot, interaction, guild, args) {
		...
		if (['pc', 'xbox', 'ps4'].includes(platform?.toLowerCase()))
			device = platforms[platform.toLowerCase()]

		// Get region
		let Region = regions['eu']
		if (['eu', 'na', 'as'].includes(region?.toLowerCase()))
			Region = regions[region.toLowerCase()]
	...
	async fetchUserData(bot, guild, channel, player, platform, region) {
		...
		const { current, max } =
			playerRank[0].seasons[Object.keys(playerRank[0].seasons)[0]].regions[region].boards
				.pvp_ranked
		const { pvp, pve } = playerStats[0]
		const { level, xp } = playerGame[0]
		platform = Object.keys(platforms)
			.find((key) => platforms[key] === platform)
			.toLowerCase()
		region = Object.keys(regions)
			.find((key) => regions[key] === region)
			.toLowerCase()

		return new Embed(bot, guild)
			.setAuthor(player.username, bot.user.displayAvatarURL)
			.setDescription(
				guild.translate('searcher/r6:DESC', {
			...
			.addField(
				guild.translate('searcher/r6:GENERAL'),
				guild.translate('searcher/r6:GEN_DATA', {
					LVL: level,
					XP: xp.toLocaleString(guild.settings.Language),
					NAME: current.name,
					MAX_NAME: max.name,
					MMR: current.mmr.toLocaleString(guild.settings.Language),
				})
			)
			.addField(
				guild.translate('searcher/r6:STATS'),
				guild.translate('searcher/r6:STAT_DATA', {
					WIN: pvp.general.wins.toLocaleString(guild.settings.Language),
					LOSS: pvp.general.losses.toLocaleString(guild.settings.Language),
					WL: (pvp.general.wins / pvp.general.matches).toFixed(2),
					KILL: pvp.general.kills.toLocaleString(guild.settings.Language),
					DEATH: pvp.general.deaths.toLocaleString(guild.settings.Language),
					KD: (pvp.general.kills / pvp.general.deaths).toFixed(2),
					TIME: Math.round(pvp.general.playtime / 3600).toLocaleString(
						guild.settings.Language
					),
				})
			)
			.addField(
				guild.translate('searcher/r6:TERRORIST'),
				guild.translate('searcher/r6:STAT_DATA', {
					WIN: pve.general.wins.toLocaleString(guild.settings.Language),
					LOSS: pve.general.losses.toLocaleString(guild.settings.Language),
					WL: (pve.general.wins / pve.general.matches).toFixed(2),
					KILL: pve.general.kills.toLocaleString(guild.settings.Language),
					DEATH: pve.general.deaths.toLocaleString(guild.settings.Language),
					KD: (pve.general.kills / pve.general.deaths).toFixed(2),
					TIME: Math.round(pve.general.playtime / 3600).toLocaleString(
						guild.settings.Language
					),
				})
			)
