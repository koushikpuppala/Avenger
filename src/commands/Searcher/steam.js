...
class Steam extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/steam:USAGE')),
				})
		...
		const resp = await this.fetchSteamData(
			bot,
			message.guild,
			message.channel,
			token,
	...
	async fetchSteamData(bot, guild, channel, token, username) {
		const { response } = await fetch(
			`http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${token}&vanityurl=${username}`
		)
			.then((res) => res.json())
			.catch((err) => {
		...
		// fetch profile data
		const {
			response: { players: resp },
		} = await fetch(
			`http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${token}&steamids=${response.steamid}`
		)
			.then((res) => res.json())
			.catch((err) => {
				bot.logger.error(`Command: 'steam' has error: ${err.message}.`)
		...
		// Check for user bans
		const { players: bans } = await fetch(
			`http://api.steampowered.com/ISteamUser/GetPlayerBans/v1/?key=${token}&steamids=${response.steamid}`
		)
			.then((res) => res.json())
			.catch((err) => {
				bot.logger.error(`Command: 'steam' has error: ${err.message}.`)
		...
		return new Embed(bot, guild)
			.setColor(0x0099ff)
			.setAuthor(
				guild.translate('searcher/steam:AUTHOR', { NAME: resp[0].personaname }),
				resp[0].avatarfull
			)
			.setThumbnail(resp[0].avatarfull)
			.setDescription(
				guild.translate('searcher/steam:DESC', {
					NAME: resp[0].realname || 'Unknown',
					STATUS: guild.translate('searcher/steam:STATE', { returnObjects: true })[
						resp[0].personastate
					],
					FLAG: resp[0].loccountrycode ? resp[0].loccountrycode.toLowerCase() : 'white',
					TIME: dateFormat(resp[0].timecreated * 1000, 'd/mm/yyyy (h:MM:ss TT)'),
					GAME_BANS: bans[0].NumberOfGameBans,
					VAC_BANS: bans[0].NumberOfVACBans,
					URL: resp[0].profileurl,
				})
			)
			.setTimestamp()
	}
