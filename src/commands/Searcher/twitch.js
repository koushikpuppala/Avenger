...
class Twitch extends Command {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/twitch:USAGE')),
				})
		...
		try {
			...
			if (twitchUser) {
				...
				const embed = new Embed(bot, message.guild)
					...
					.addField(
						message.translate('searcher/twitch:BIO'),
						twitchUser.description || message.translate('searcher/twitch:NO_BIO'),
						true
					)
					.addField(
						message.translate('searcher/twitch:TOTAL'),
						twitchUser.view_count.toLocaleString(settings.Language),
						true
					)
					.addField(
						message.translate('searcher/twitch:FOLLOWERS'),
						await this.getFollowersFromId(bot, twitchUser.id).then((num) =>
							num.toLocaleString(settings.Language)
						),
						true
					)
				if (stream) {
					embed
						.addField(
							'\u200B',
							message.translate('searcher/twitch:STREAMING', {
								TITLE: stream.title,
								NUM: stream.viewer_count,
							})
						)
						.setImage(
		...
		try {
			...
			if (twitchUser) {
				...
				const embed = new Embed(bot, guild)
					...
					.setAuthor('Twitch', 'https://i.imgur.com/4b9X738.png')
					.addField(
						guild.translate('searcher/twitch:BIO'),
						twitchUser.description || guild.translate('searcher/twitch:NO_BIO'),
						true
					)
					.addField(
						guild.translate('searcher/twitch:TOTAL'),
						twitchUser.view_count.toLocaleString(guild.settings.Language),
						true
					)
					.addField(
						guild.translate('searcher/twitch:FOLLOWERS'),
						await this.getFollowersFromId(bot, twitchUser.id).then((num) =>
							num.toLocaleString(guild.settings.Language)
						),
						true
					)
				if (stream) {
					embed
						.addField(
							'\u200B',
							guild.translate('searcher/twitch:STREAMING', {
								TITLE: stream.title,
								NUM: stream.viewer_count,
							})
						)
				...
				interaction.reply({
					embeds: [channel.error('searcher/twitch:NOT_FOUND', {}, true)],
				})
			}
		} catch (err) {
			bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`)
	...
	request(bot, endpoint, queryParams = {}) {
		...
		return fetch('https://api.twitch.tv/helix' + endpoint + `?${qParams.toString()}`, {
			headers: {
				'Client-ID': bot.config.api_keys.twitch.clientID,
			...
			.then((data) => {
				if (data.error === 'Unauthorized') {
					return this.refreshTokens(bot).then(() =>
						this.request(bot, endpoint, queryParams)
					)
				}
				return data
			})
	...
	async refreshTokens(bot) {
		await fetch(
			`https://id.twitch.tv/oauth2/token?client_id=${bot.config.api_keys.twitch.clientID}&client_secret=${bot.config.api_keys.twitch.clientSecret}&grant_type=client_credentials`,
			{
				method: 'POST',
			}
		)
