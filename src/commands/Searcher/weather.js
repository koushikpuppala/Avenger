...
class Weather extends Command {
	...
	async run(bot, message, settings) {
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('searcher/weather:USAGE')),
				})
		...
		await find({ search: message.args.join(' '), degreeType: 'C' }, (err, result) => {
			...
			if (!result[0])
				return message.channel
					.error('search/weather:INVALID')
					.then((m) => m.timedDelete({ timeout: 5000 }))

			// Display weather at location
			const embed = new Embed(bot, message.guild)
				.setTitle(
					message.translate('searcher/weather:TITLE', { LOC: result[0].location.name })
				)
				.setDescription(message.translate('searcher/weather:DESC'))
				.addField(
					message.translate('searcher/weather:TEMP'),
					`${result[0].current.temperature}°C`,
					true
				)
				.addField(
					message.translate('searcher/weather:SKY'),
					result[0].current.skytext,
					true
				)
				.addField(
					message.translate('searcher/weather:HUMIDITY'),
					`${result[0].current.humidity}%`,
					true
				)
				.addField(
					message.translate('searcher/weather:SPEED'),
					result[0].current.windspeed,
					true
				)
				.addField(
					message.translate('searcher/weather:TIME'),
					result[0].current.observationtime,
					true
				)
				.addField(
					message.translate('searcher/weather:DISPLAY'),
	...
	async callback(bot, interaction, guild, args) {
		...
		await find({ search: location, degreeType: 'C' }, (err, result) => {
			...
			const embed = new Embed(bot, guild)
				.setTitle(
					guild.translate('searcher/weather:TITLE', { LOC: result[0].location.name })
				)
				.setDescription(guild.translate('searcher/weather:DESC'))
				.addField(
					guild.translate('searcher/weather:TEMP'),
					`${result[0].current.temperature}°C`,
					true
				)
				.addField(guild.translate('searcher/weather:SKY'), result[0].current.skytext, true)
				.addField(
					guild.translate('searcher/weather:HUMIDITY'),
					`${result[0].current.humidity}%`,
					true
				)
				.addField(
					guild.translate('searcher/weather:SPEED'),
					result[0].current.windspeed,
					true
				)
				.addField(
					guild.translate('searcher/weather:TIME'),
					result[0].current.observationtime,
					true
				)
				.addField(
					guild.translate('searcher/weather:DISPLAY'),
					result[0].current.winddisplay,
					true
				)
				.setThumbnail(result[0].current.imageUrl)
