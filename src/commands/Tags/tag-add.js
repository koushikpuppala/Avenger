...
class TagAdd extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('tags/tag-add:USAGE')),
				})
		...
		try {
			...
			if (!message.args[0])
				return message.channel.send(message.translate('tags/tag-add:INVALID_NAME'))
			if (!message.args[0].length > 10)
				return message.channel.send(message.translate('tags/tag-add:NAME_TOO_LONG'))
			if (!responseString)
				return message.channel.send(message.translate('tags/tag-add:INVALID_RESP'))
			...
			TagsSchema.find({ guildID: message.guild.id }).then(async (guildTags) => {
				if (guildTags.length >= 10 && !message.guild.premium)
					return message.channel.send(message.translate('tags/tag-add:NEED_PREMIUM'))
				if (guildTags.length >= 50)
					return message.channel.send(message.translate('tags/tag-add:MAX_TAGS'))

				// Make sure the tagName doesn't exist
				for (const tag of guildTags) {
					// tagName already exists
					if (tag.name == message.args[0])
				...
				message.channel.send(
					message.translate('tags/tag-add:TAGS_SAVED', { TAG: message.args[0] })
				)
