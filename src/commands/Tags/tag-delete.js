		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('tags/tag-delete:USAGE')),
				})
				.then((m) => m.timedDelete({ timeout: 5000 }))

		// try and delete tag
		try {
			const result = await TagsSchema.findOneAndRemove({
				guildID: message.guild.id,
				name: message.args[0],
			})
			if (result) {
				message.channel.send(
					message.translate('tags/tag-delete:TAG_DELETED', { TAG: message.args[0] })
				)
				message.guild.guildTags.splice(message.guild.guildTags.indexOf(message.args[0]), 1)
			} else {
				message.channel.send(
					message.translate('tags/tag-delete:NO_TAG', { TAG: message.args[0] })
				)
			}
		} catch (err) {
