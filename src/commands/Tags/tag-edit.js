...
class TagEdit extends Command {
	...
	async run(bot, message, settings) {
		...
		if (!message.args[0])
			return message.channel
				.error('misc:INCORRECT_FORMAT', {
					EXAMPLE: settings.prefix.concat(message.translate('tags/tag-edit:USAGE')),
				})
		...
		if (message.args[0].toLowerCase() == 'rename') {
			...
			if (!message.args[1])
				return message.channel.send(message.translate('tags/tag-edit:INVALID_NAME'))
			if (!message.args[2])
				return message.channel.send(message.translate('tags/tag-edit:INVALID_NEW_NAME'))
			try {
				await TagsSchema.findOneAndUpdate(
					{ guildID: message.guild.id, name: message.args[1] },
					{ name: message.args[2] }
				).then(() => {
					message.channel.send(
						message.translate('tags/tag-edit:UPDATED_NAME', { NAME: message.args[2] })
					)
					message.guild.guildTags.splice(
						message.guild.guildTags.indexOf(message.args[1]),
						1
					)
			...
			if (!message.args[1])
				return message.channel.send(message.translate('tags/tag-edit:INVALID_NAME'))
			if (!responseString)
				return message.channel.send(message.translate('tags/tag-edit:INVALID_NEW_RESP'))
			try {
				await TagsSchema.findOneAndUpdate(
					{ guildID: message.guild.id, name: message.args[1] },
					...
					message.channel.send(
						message.translate('tags/tag-edit:UPDATED_RESP', { NAME: message.args[2] })
					)
				})
			} catch (err) {
				bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`)
