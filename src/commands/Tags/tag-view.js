...
class TagView extends Command {
	...
	async run(bot, message) {
		...
		TagsSchema.find({ guildID: message.guild.id }).then((result) => {
			...
			if (result != null) {
				const resultEmbed = new Embed(bot, message.guild).setTitle('tags/tag-view:TITLE', {
					NAME: message.guild.name,
				})
				result.forEach((value) => {
					resultEmbed.addField(
						message.translate('tags/tag-view:TITLE', { NAME: value.name }),
						message.translate('tags/tag-view:RESP', { RESP: value.response })
					)
				})
