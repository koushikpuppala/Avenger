	...
	async run(bot, message, settings) {
		...
		if (
			...
			return message.channel
				.error('ticket/ticket-create:TICKET_EXISTS')
		...
		if (message.guild.roles.cache.get(settings.TicketSupportRole))
			perms.push({ id: settings.TicketSupportRole, allow: ['SEND_MESSAGES', 'VIEW_CHANNEL'] })
		...
		message.guild.channels
			...
			.then(async (channel) => {
				...
				const successEmbed = new Embed(bot, message.guild)
					...
					.setDescription(
						message.translate('ticket/ticket-create:DESC', { CHANNEL: channel.id })
					)
				message.channel
				...
				const embed = new Embed(bot, message.guild)
					.setColor(0xff5555)
					.addField(
						message.translate('ticket/ticket-create:FIELD1', {
							USERNAME: message.author.tag,
						}),
						message.translate('ticket/ticket-create:FIELDT')
					)
					.addField(message.translate('ticket/ticket-create:FIELD2'), reason)
					.setTimestamp()
				channel.send({
					content: `${message.author}${
						message.guild.roles.cache.get(settings.TicketSupportRole)
