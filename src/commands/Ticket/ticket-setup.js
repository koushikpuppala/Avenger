	...
	async run(bot, message, settings) {
		...
		if (!message.args[0]) {
			...
			try {
				...
				if (!channel || channel.type != 'GUILD_CATEGORY')
					return message.channel.send(
						message.translate('ticket/ticket-setup:NOT_CATEGORY')
					)
				// update database
				await message.guild.updateGuild({ TicketCategory: message.args[1] })
				message.channel.send(
					message.translate('ticket/ticket-setup:UPDATED_CATEGORY', {
						NAME: channel.name,
					})
				)
			} catch (err) {
			...
			try {
				...
				if (!supportRole)
					return message.channel.send(message.translate('ticket/ticket-setup:NOT_ROLE'))
				// update database
				await message.guild.updateGuild({ TicketSupportRole: message.args[1] })
				message.channel.send(
					message
						.translate('ticket/ticket-setup:UPDATED_ROLE')
						.replace('{ROLE}', supportRole)
				)
			} catch (err) {
				if (message.deletable) message.delete()
				bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`)
