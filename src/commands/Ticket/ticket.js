		...
		if (message.member.permissions.has('MANAGE_GUILD')) {
			if (message.args[0] == 'reaction') {
				const embed = new Embed(bot, message.guild)
					...
					.setDescription(
						message.translate('ticket/ticket:REACT_DESC', { PREFIX: settings.prefix })
					)
				// Create button
				...
				const embed = new Embed(bot, message.guild)
					...
					.setDescription(
						[
							`\`${settings.prefix}t-<open|create> [reason]\` - ${message.translate(
								'ticket/ticket-create:DESCRIPTION'
							)}.`,
							`\`${settings.prefix}t-close\` - ${message.translate(
								'ticket/ticket-close:DESCRIPTION'
							)} (Support only).`,
							`\`${settings.prefix}t-setup\` - ${message.translate(
								'ticket/ticket-setup:DESCRIPTION'
							)} (Admin only).`,
