	...
	async run(bot, button) {
		...
		if (ID == 'crt_ticket') {
			...
			if (guild.channels.cache.find((c) => c.name == `ticket-${member.user.id}`)) {
				return button
					.reply({
						embeds: [channel.error('ticket/ticket-create:TICKET_EXISTS', {}, true)],
					})
					.then(() => {
						setTimeout(function () {
							button.deleteReply()
			...
			if (guild.roles.cache.get(guild.settings.TicketSupportRole))
				perms.push({
					id: guild.settings.TicketSupportRole,
					allow: ['SEND_MESSAGES', 'VIEW_CHANNEL'],
				})

			// create channel
			guild.channels
				.create(`ticket-${member.user.id}`, {
					type: 'text',
				...
				.then(async (c) => {
					...
					const successEmbed = new Embed(bot, guild)
						...
						.setDescription(
							guild.translate('ticket/ticket-create:DESC', { CHANNEL: channel.id })
						)
					...
					const embed = new Embed(bot, guild)
						.setColor(0xff5555)
						.addField(
							guild.translate('ticket/ticket-create:FIELD1', {
								USERNAME: member.user.tag,
							}),
							guild.translate('ticket/ticket-create:FIELDT')
						)
						.addField(
							guild.translate('ticket/ticket-create:FIELD2'),
							guild.translate('misc:NO_REASON')
						)
						.setTimestamp()
					c.send({
						content: `${member.user}${
							guild.roles.cache.get(guild.settings.TicketSupportRole)
								? `, <@&${guild.settings.TicketSupportRole}>`
								: ''
						}.`,
						embeds: [embed],
					})
