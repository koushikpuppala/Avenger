...
class SlashCreate extends Event {
	...
	async run(bot, interaction) {
		...
		if (
			guild.settings.CommandChannelToggle &&
			guild.settings.CommandChannels.includes(channel.id)
		) {
			return interaction.reply({
				embeds: [
					channel.error(
						'events/message:BLACKLISTED_CHANNEL',
		...
		if (neededPermissions.length > 0) {
			bot.logger.error(
				`Missing permission: \`${neededPermissions.join(', ')}\` in [${guild.id}].`
			)
			return interaction.reply({
				embeds: [
					channel.error(
						'misc:MISSING_PERMISSION',
						{
							PERMISSIONS: neededPermissions
								.map((p) => bot.translate(`permissions:${p}`))
								.join(', '),
						},
						true
					),
		...
		if (timestamps.has(interaction.user.id)) {
			...
			if (now < expirationTime) {
				...
				return interaction.reply({
					embeds: [
						channel.error(
							'events/message:COMMAND_COOLDOWN',
