...
class ChannelUpdate extends Event {
	...
	async run(bot, oldChannel, newChannel) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Channel: ${
					newChannel.type == 'dm' ? newChannel.recipient.tag : newChannel.name
				} has been updated${
					newChannel.type == 'dm' ? '' : ` in guild: ${newChannel.guild.id}`
				}. (${types[newChannel.type]})`
			)
		...
		if (settings.ModLogEvents?.includes('CHANNELUPDATE') && settings.ModLog) {
			...
			if (oldChannel.name != newChannel.name) {
				embed = new Embed(bot, newChannel.guild)
					.setDescription(
						`**${
							newChannel.type === 'GUILD_CATEGORY' ? 'Category' : 'Channel'
						} name changed of ${newChannel.toString()}**`
					)
					.setColor(15105570)
			...
			if (oldChannel.topic != newChannel.topic) {
				embed = new Embed(bot, newChannel.guild)
					.setDescription(
						`**${
							newChannel.type === 'GUILD_CATEGORY' ? 'Category' : 'Channel'
						} topic changed of ${newChannel.toString()}**`
					)
					.setColor(15105570)
					.setFooter(`ID: ${newChannel.id}`)
					.setAuthor(newChannel.guild.name, newChannel.guild.iconURL())
			...
			if (oldChannel.rtcRegion != newChannel.rtcRegion) {
				embed = new Embed(bot, newChannel.guild)
					.setDescription(
						`**${
							newChannel.type === 'GUILD_CATEGORY' ? 'Category' : 'Channel'
						} region changed of ${newChannel.toString()}**`
					)
					.setColor(15105570)
					.setFooter(`ID: ${newChannel.id}`)
					.setAuthor(newChannel.guild.name, newChannel.guild.iconURL())
			...
			const permDiff = oldChannel.permissionOverwrites.cache
				.filter((x) => {
					if (
						newChannel.permissionOverwrites.cache.find(
				...
				.concat(
					newChannel.permissionOverwrites.cache.filter((x) => {
						if (
							oldChannel.permissionOverwrites.cache.find(
								(y) => y.allow.bitfield == x.allow.bitfield
							) &&
							oldChannel.permissionOverwrites.cache.find(
								(y) => y.deny.bitfield == x.deny.bitfield
			...
			if (updated) {
				...
				try {
					const modChannel = await bot.channels
						...
						.catch(() =>
							bot.logger.error(
								`Error fetching guild: ${newChannel.guild.id} logging channel`
							)
						)
