...
class EmojiUpdate extends Event {
	...
	async run(bot, oldEmoji, newEmoji) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Emoji: ${newEmoji.name} has been updated in guild: ${newEmoji.guild.id}.`
			)
		...
		if (settings.ModLogEvents?.includes('EMOJIUPDATE') && settings.ModLog) {
			...
			if (oldEmoji.roles.cache.size != newEmoji.roles.cache.size) {
				const rolesAdded = newEmoji.roles.cache.filter(
					(x) => !oldEmoji.roles.cache.get(x.id)
				)
				const rolesRemoved = oldEmoji.roles.cache.filter(
					(x) => !newEmoji.roles.cache.get(x.id)
				)
						...
						.addFields(
							{
								...
								value: `${
									roleAddedString.length == 0
										? '*None*'
										: roleAddedString.join('\n ')
								}`,
								inline: true,
							},
							{
								...
								value: `${
									roleRemovedString.length == 0
										? '*None*'
										: roleRemovedString.join('\n ')
								}`,
								inline: true,
							}
						)
			...
			if (updated) {
				...
				try {
					const modChannel = await bot.channels
						...
						.catch(() =>
							bot.logger.error(
								`Error fetching guild: ${newEmoji.guild.id} logging channel`
							)
						)
