 ...
 * @event Avenger#GuildMemberRemove
 * @extends {Event}
 */
	...
	async run(bot, member) {
		...
		if (settings.ModLogEvents?.includes('GUILDMEMBERREMOVE') && settings.ModLog) {
			const embed = new Embed(bot, member.guild)
				...
				.addField(
					...
					member.partial
						? 'Unknown'
						: `${dateFormat(member.joinedAt, 'ddd dd/mm/yyyy')} (${Math.round(
								(new Date() - member.joinedAt) / 86400000
						  )} day(s) ago)`
				)
				.setTimestamp()
			...
			try {
				const modChannel = await bot.channels
					...
					.catch(() =>
						bot.logger.error(`Error fetching guild: ${member.guild.id} logging channel`)
					)
		...
		if (settings.welcomePlugin) {
			...
			if (channel && settings.welcomeGoodbyeToggle)
				channel
					.send(settings.welcomeGoodbyeText.replace('{user}', member.user))
