 ...
 * @event Avenger#GuildMemberUpdate
 * @extends {Event}
 */
class GuildMemberUpdate extends Event {
	...
	async run(bot, oldMember, newMember) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Member: ${newMember.user.tag} has been updated in guild: ${newMember.guild.id}.`
			)
					...
					.addFields(
						{
							...
							value: `${
								roleAddedString.length == 0 ? '*None*' : roleAddedString.join('\n ')
							}`,
							inline: true,
						},
						{
			...
			if (updated) {
				...
				try {
					const modChannel = await bot.channels
						...
						.catch(() =>
							bot.logger.error(
								`Error fetching guild: ${newMember.guild.id} logging channel`
							)
						)
		...
		if (
			oldMember.roles.cache
				...
				.map((r) => r.id)
				.includes(settings.MutedRole)
		) {
			try {
				await newMember.guild.updateGuild({
					MutedMembers: settings.MutedMembers.filter((user) => user != newMember.user.id),
				})
