		...
		if (settings.ModLog && settings.ModLogEvents?.includes('GUILDUPDATE')) {
			...
			if (oldGuild.premiumTier != newGuild.premiumTier) {
				embed = new Embed(bot, newGuild)
					.setDescription(
						`**Server boost ${
							oldGuild.premiumTier < newGuild.premiumTier ? 'increased' : 'decreased'
						}**`
					)
					.setAuthor(newGuild.name, newGuild.iconURL())
			...
			if (updated) {
				...
				try {
					const modChannel = await bot.channels
						...
						.catch(() =>
							bot.logger.error(`Error fetching guild: ${newGuild.id} logging channel`)
						)
