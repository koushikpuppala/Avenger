...
class MessageDelete extends Event {
	...
	async run(bot, message) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Message has been deleted${!message.guild ? '' : ` in guild: ${message.guild.id}`}.`
			)
		...
		try {
			...
			const rr = await ReactionRoleSchema.findOneAndRemove({
			...
			// check giveaways
			const g = await GiveawaySchema.findOneAndRemove({
				messageID: message.id,
				channelID: message.channel.id,
			})
			if (g) {
		...
		if (settings.ModLogEvents?.includes('MESSAGEDELETE') && settings.ModLog) {
			...
			const embed = new Embed(bot, message.guild)
				.setDescription(
					`**Message from ${message.author.toString()} deleted in ${message.channel.toString()}**`
				)
				.setColor(15158332)
				.setFooter(`Author: ${message.author.id} | Message: ${message.id}`)
				.setAuthor(message.author.tag, message.author.displayAvatarURL())
			if (message.content.length > 0)
				embed.addField(`Content ${shortened ? ' (shortened)' : ''}:`, `${content}`)
			embed.setTimestamp()
			...
			try {
				const modChannel = await bot.channels
					...
					.catch(() =>
						bot.logger.error(
							`Error fetching guild: ${message.guild.id} logging channel`
						)
					)
