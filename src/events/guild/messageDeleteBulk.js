 ...
 * @event Avenger#MessageDeleteBulk
 * @extends {Event}
 */
class MessageDeleteBulk extends Event {
	...
	async run(bot, messages) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`${messages.size} messages have been deleted in guild: ${messages.first().guild.id}`
			)
		...
		if (settings.ModLogEvents?.includes('MESSAGEDELETEBULK') && settings.ModLog) {
			...
			let humanLog = `**Deleted Messages from #${messages.first().channel.name} (${
			...
			for (const message of [...messages.values()].reverse()) {
				...
				)}] ${message.author?.tag ?? 'Unknown'} (${message.id})`
			...
			if (modChannel) {
				...
				const embed = new Embed(bot, messages.first().guild.id)
					.setDescription(
						`**Bulk deleted messages in ${messages.first().channel.toString()}**`
					)
					.setColor(15158332)
					...
					.addField(
						'Deleted Messages:',
						`[view](https://txt.discord.website/?txt=${modChannel.id}/${
