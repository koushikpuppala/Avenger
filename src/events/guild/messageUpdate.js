...
class MessageUpdate extends Event {
	...
	async run(bot, oldMessage, newMessage) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Message updated${!newMessage.guild ? '' : ` in guild: ${newMessage.guild.id}`}`
			)
		...
		if (settings.ModLogEvents?.includes('MESSAGEUPDATE') && settings.ModLog) {
			...
			const embed = new Embed(bot, newMessage.guild)
				.setDescription(
					`**Message of ${newMessage.author.toString()} edited in ${newMessage.channel.toString()}** [Jump to Message](${
						newMessage.url
					})`
				)
			...
			try {
				const modChannel = await bot.channels
					...
					.catch(() =>
						bot.logger.error(
							`Error fetching guild: ${newMessage.guild.id} logging channel`
						)
					)
