...
class StickerCreate extends Event {
	...
	async run(bot, sticker) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Sticker: ${sticker.name} has been created in guild: ${sticker.guildId}. (${sticker.type})`
			)
		...
		if (settings.ModLogEvents?.includes('STICKERCREATE') && settings.ModLog) {
			const embed = new Embed(bot, sticker.guild)
				.setDescription(
					[
						`**Sticker created: ${sticker.name}**`,
						`${user ? ['', `Created by: ${user?.tag}`].join('\n') : []}`,
					].join('\n')
				)
				.setColor(3066993)
			...
			try {
				const modChannel = await bot.channels
					...
					.catch(() =>
						bot.logger.error(`Error fetching guild: ${sticker.guildId} logging channel`)
					)
