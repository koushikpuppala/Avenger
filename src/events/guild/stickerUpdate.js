...
class StickerUpdate extends Event {
	...
	async run(bot, oldSticker, newSticker) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Sticker: ${newSticker.name} has been updated in guild: ${newSticker.guildId}. (${newSticker.type})`
			)
			...
			if (updated) {
				try {
					const modChannel = await bot.channels
						...
						.catch(() =>
							bot.logger.error(
								`Error fetching guild: ${newSticker.guildId} logging channel`
							)
						)
