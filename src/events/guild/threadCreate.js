...
class ThreadCreate extends Event {
	...
	async run(bot, thread) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Thread: ${thread.name} has been created in guild: ${thread.guildId}. (${
		...
		if (settings.ModLogEvents?.includes('THREADCREATE') && settings.ModLog) {
			const embed = new Embed(bot, thread.guild)
				.setDescription(
					[
						`**${thread.type
							.split('_')[1]
							.toLowerCase()} thread created: ${thread.toString()}**`,
						'',
			...
			try {
				const modChannel = await bot.channels
					...
					.catch(() =>
						bot.logger.error(`Error fetching guild: ${thread.guild.id} logging channel`)
					)
