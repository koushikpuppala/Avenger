 ...
 * @event Avenger#ThreadMembersUpdate
 * @extends {Event}
 */
class ThreadMembersUpdate extends Event {
	...
	async run(bot, oldMembers, newMembers) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Thread: ${thread.name} member count has been updated in guild: ${
					thread.guildId
				}. (${thread.type.split('_')[1]})`
			)
			...
			if (oldMembers.size != newMembers.size) {
				...
				if (memberAdded.size != 0 || memberRemoved.size != 0) {
					...
					const embed = new Embed(bot, thread.guild)
						...
						.addFields(
							{
								name: `Added members [${memberAdded.size}]:`,
								value: `${
									memberAddedString.length == 0
										? '*None*'
										: memberAddedString.join('\n ')
								}`,
								inline: true,
							},
							{
								name: `Removed member [${memberRemoved.size}]:`,
								value: `${
									memberRemovedString.length == 0
										? '*None*'
										: memberRemovedString.join('\n ')
								}`,
								inline: true,
							}
						)
					...
					try {
						const modChannel = await bot.channels
							...
							.catch(() =>
								bot.logger.error(
									`Error fetching guild: ${thread.guildId} logging channel`
								)
							)
