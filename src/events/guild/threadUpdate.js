...
class ThreadUpdate extends Event {
	...
	async run(bot, oldThread, newThread) {
		...
		if (bot.config.debug)
			bot.logger.debug(
				`Thread: ${newThread.name} has been updated in guild: ${newThread.guildId}. (${
					newThread.type.split('_')[1]
				})`
			)
			...
			if (updated) {
				try {
					const modChannel = await bot.channels
						...
						.catch(() =>
							bot.logger.error(
								`Error fetching guild: ${newThread.guild.id} logging channel`
							)
						)
