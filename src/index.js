...
(async function load() {
	...
	const configCorrect = await require('./validations/verify-config.js').run(
		require('./config.js'),
	);
	...
	// This file is for sharding
		...
		manager
			.spawn()
			.then(logger.log('=-=-=-=-=-=-=- Loading shard(s) -=-=-=-=-=-=-='));
		...
		Sentry.init({
			dsn: require('./config.js').dsn,
			environment: require('./config.js').environment,
		});
