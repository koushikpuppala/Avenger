...
module.exports.run = async (config) => {
	figlet('Avengers Assemble', (err, data) => {
		if (err) {
	...
	if (config.api_keys.twitch.clientID && config.api_keys.twitch.clientSecret) {
		const req = await fetch(
			`https://id.twitch.tv/oauth2/token?client_id=${config.api_keys.twitch.clientID}&client_secret=${config.api_keys.twitch.clientSecret}&grant_type=client_credentials`,
			{
				method: 'POST',
			}
		)
	...
	if (!config.api_keys.fortnite) {
		...
		const stats = await new (require('../../APIs/fortnite.js'))(config.api_keys.fortnite).user(
			'Ninja',
			'pc'
		)
	...
	if (!config.api_keys.steam) {
		...
		try {
			await fetch(
				`http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=${config.api_keys.steam}&vanityurl=eroticgaben`
			).then((res) => res.json())
		} catch (e) {
